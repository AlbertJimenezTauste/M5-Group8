...
def mapper(dataset_dict):
    ddataset_dict = copy.deepcopy(dataset_dict)  # it will be modified by code below
    image = utils.read_image(dataset_dict["file_name"], format="BGR")
    #image, transforms = T.apply_transform_gens([T.RandomFlip(prob=0.5, horizontal=True, vertical=False), T.RandomContrast(intensity_min=0.5, intensity_max=2)], image)
    image, transforms = T.apply_transform_gens([T.RandomFlip(prob=0.5, horizontal=True, vertical=False)], image)
...
short_model_name = short_model_name + '_flip2'
...
os.makedirs("../" + short_model_name + "/output/validation", exist_ok=True)
os.makedirs("../" + short_model_name + "/output/test", exist_ok=True)
...
evaluator = COCOEvaluator("KITTI_MOTS_val", cfg, False, output_dir=os.path.join(cfg.OUTPUT_DIR,'val'))
val_loader = build_detection_test_loader(cfg, "KITTI_MOTS_val")
inference_on_dataset(predictor.model, val_loader, evaluator)
model_trained = predictor.model 
torch.save(model_trained.state_dict(), 'trained_model_sim_.pth')