...
KITTI_metadata = MetadataCatalog.get("KITTI_MOTS_test")

models = ['trained_model_sim_1.pth','trained_model_sim_0.8.pth',
            'trained_model_sim_0.6.pth','trained_model_sim_0.4.pth',
            'trained_model_sim_0.2.pth','trained_model_sim_0.1.pth']

dataset_dicts = get_KITTI_MOTS_dicts("/home/mcv/datasets/KITTI-MOTS/training/image_02/", test_seqmap)

for model_n in models:
    model_name = 'COCO-InstanceSegmentation/mask_rcnn_R_50_FPN_3x.yaml'
    dirname = model_n.split(".pth")[0]
    print(dirname)
    os.makedirs("../" + dirname + '/inference', exist_ok=True)
    ...
    model.load_state_dict(torch.load("../" + model_n))
    predictor.model = model
    cfg.OUTPUT_DIR = "../" + dirname
    evaluator = COCOEvaluator("KITTI_MOTS_test", cfg, False, output_dir=os.path.join(cfg.OUTPUT_DIR))
    ...
    for d in random.sample(dataset_dicts, 20):    
        ...
        cv2.imwrite("../" + dirname + "/inference/" + str(ide) +".jpg", v.get_image()[:, :, ::-1])
        ide += 1
...
percentages = [1, 0.8, 0.6, 0.4, 0.2, 0.1]
