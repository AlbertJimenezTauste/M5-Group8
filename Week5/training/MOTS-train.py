...
def get_dicts(img_dir, seqmap, dataset):

    if dataset == "MOTS":
        path = '/home/grupo08/datasets/MOTSChallenge/instances_txt/'
        file_format = '.jpg'
    elif dataset == "KITTI-MOTS":
    ...
    for seq in seqmap:
        ...
        for i in ids:
            ...
            record["file_name"] = img_dir + seq + '/' + f'{i:06}' + file_format
...
# Sequences to load:
train_seqmap_MOTS = ['0002','0005','0009','0011']

val_seqmap_KITTI_MOTS = ['0002','0006','0007','0008','0010','0013','0014','0016','0018']


# Create the dataset
d = "train"
DatasetCatalog.register("MOTS_" + d, 
    lambda d=d: get_dicts("/home/mcv/datasets/MOTSChallenge/train/images/", train_seqmap_MOTS, "MOTS"))
MetadataCatalog.get("MOTS_" + d).set(thing_classes=class_list)

d = "val"
DatasetCatalog.register("KITTI_MOTS_" + d, 
    lambda d=d: get_dicts("/home/mcv/datasets/KITTI-MOTS/training/image_02/", val_seqmap_KITTI_MOTS, "KITTI-MOTS"))
MetadataCatalog.get("KITTI_MOTS_" + d).set(thing_classes=class_list)

MOTS_metadata_train = MetadataCatalog.get("MOTS_train")
KITTI_MOTS_metadata_val = MetadataCatalog.get("KITTI_MOTS_val")
#print(MOTS_metadata_train)
#print(KITTI_MOTS_metadata_val)


# Verify it is well loaded
MOTS_dict_train = get_dicts("/home/mcv/datasets/MOTSChallenge/train/images/", train_seqmap_MOTS, "MOTS")

ide = 0
for d in random.sample(MOTS_dict_train, 10):
    img = cv2.imread(d["file_name"])
    visualizer = Visualizer(img[:, :, ::-1], metadata=MOTS_metadata_train, scale=0.5)
...
KITTI_MOTS_dict_val = get_dicts("/home/mcv/datasets/KITTI-MOTS/training/image_02/", val_seqmap_KITTI_MOTS, "KITTI-MOTS")
ide = 0
for d in random.sample(KITTI_MOTS_dict_val, 20):    
    im = cv2.imread(d["file_name"])
    outputs = predictor(im)
    v = Visualizer(im[:, :, ::-1],
                    metadata=MOTS_metadata_train, 
                    scale=0.8
    )
