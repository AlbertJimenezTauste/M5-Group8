...
models = ['Cityscapes/mask_rcnn_R_50_FPN.yaml', 'COCO-InstanceSegmentation/mask_rcnn_R_50_FPN_3x.yaml']
...
for model_name in models:
    ...
    os.makedirs("../" + short_model_name + '_NOT_TRAINED' + "/ground_truth", exist_ok=True)
    os.makedirs("../" + short_model_name + '_NOT_TRAINED' + "/masks", exist_ok=True)
    os.makedirs("../" + short_model_name + '_NOT_TRAINED' + "/inference", exist_ok=True)
    os.makedirs("../" + short_model_name + '_NOT_TRAINED' + "/output", exist_ok=True)
    ...
    cfg.OUTPUT_DIR = "../" + short_model_name + '_NOT_TRAINED' + "/output"
    ...
    if first:
        ...
        MetadataCatalog.get("MOTSChallenge_" + d).set(thing_classes=MetadataCatalog.get(cfg.DATASETS.TRAIN[0]).thing_classes)
    first = False
    MOTS_metadata = MetadataCatalog.get("MOTSChallenge_val")
    ...
    for d in random.sample(dataset_dicts, 2):
        ...
        cv2.imwrite("../gt_motschallenge/gt_img_"+str(ide)+".jpg", vis.get_image()[:, :, ::-1])
        ide += 1
    ...
    for d in random.sample(dataset_dicts, 20):    
        ...
        cv2.imwrite("../" + short_model_name + '_NOT_TRAINED' + "/inference/predicted"+str(ide)+".jpg", v.get_image()[:, :, ::-1])
        ide += 1
