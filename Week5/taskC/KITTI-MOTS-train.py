...
for d in random.sample(dataset_dicts, 2):
    ...
    cv2.imwrite("../ground_truth/gt_img_"+str(ide)+".jpg", vis.get_image()[:, :, ::-1])
    ide += 1
...
#Data augmentation
#CROP_SIZES = [[0.8, 0.8], [0.9, 0.9], [1.0, 1.0]]
ANCHOR_RATIOS = [[[1.0, 2.0, 0.5, 3.0, 0.3333]], [[1.0, 2.0, 0.5, 2.5, 0.4]], [[1.0, 2.0, 0.5, 4, 0.25]]]
ANCHOR_THRESHS = [[0.25, 0.75], [0.3, 0.7], [0.35, 0.65]]
BATCH_SIZES_PER_IMAGE = [128, 256, 512]

#FOR
for i in range(len(ANCHOR_RATIOS)):
    for j in range(len(ANCHOR_THRESHS)):
        for k in range(len(BATCH_SIZES_PER_IMAGE)):
            print("\n\n\n\nPARAMETERS USED TO TRAIN:\n-------------------------------")
            print("ANCHOR_RATIO: " + str(ANCHOR_RATIOS[i]) +
            "\nANCHOR_THRESHS: " + str(ANCHOR_THRESHS[j]) +
            "\nBATCH_SIZES_PER_IMAGE: " + str(BATCH_SIZES_PER_IMAGE[k]))
            print("-------------------------------\n\n\n\n")
            ...
            #cfg.INPUT.CROP.SIZE = CROP_SIZES[i]
            cfg.MODEL.ANCHOR_GENERATOR.ASPECT_RATIOS = ANCHOR_RATIOS[i]
            cfg.MODEL.RPN.IOU_THRESHOLDS = ANCHOR_THRESHS[j]
            cfg.MODEL.RPN.BATCH_SIZE_PER_IMAGE = BATCH_SIZES_PER_IMAGE[k]

            experiment_name = 'ratio_' + str(i) + '_thresh_' + str(j) + '_batch_' + str(k)  
            cfg.OUTPUT_DIR = "../" + experiment_name + "/output"
            os.makedirs("../" + experiment_name + "/masks", exist_ok=True)
            os.makedirs("../" + experiment_name + "/inference", exist_ok=True)
            os.makedirs("../" + experiment_name + "/output", exist_ok=True)
            ...
            for d in random.sample(dataset_dicts, 20):    
                ...
                cv2.imwrite("../" + experiment_name + "/inference/predicted"+str(ide)+".jpg", v.get_image()[:, :, ::-1])
                ide += 1
